...
- (void)setShowPercentage:(BOOL)showPercentage
{
    ...
    for(SliceLayer *layer in _pieView.layer.sublayers)
    {
        ...
        NSDictionary *fontDic = [NSDictionary dictionaryWithObject:self.labelFont forKey:NSFontAttributeName];
        CGSize size = [label sizeWithAttributes:fontDic];
...
- (SliceLayer *)createSliceLayer
{
    ...
    NSDictionary *fontDic = [NSDictionary dictionaryWithObject:self.labelFont forKey:NSFontAttributeName];
    CGSize size = [@"0" sizeWithAttributes:fontDic];
    [CATransaction setDisableActions:YES];
...
- (void)updateLabelForLayer:(SliceLayer *)pieLayer value:(CGFloat)value
{
    NSDictionary *attrDic = [NSDictionary dictionaryWithObject:[UIColor clearColor] forKey:NSForegroundColorAttributeName];
    ...
    {
        NSAttributedString *attrLabel = [[NSAttributedString alloc]initWithString:label attributes:attrDic];
        [textLayer setString:attrLabel];
        
        NSLog(@"textLayer string = %@",label);
        [textLayer setBounds:CGRectMake(0, 0, size.width, size.height)];
    }
    [CATransaction setDisableActions:NO];
}
